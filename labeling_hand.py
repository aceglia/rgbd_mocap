...
from utils import minimize_points_location, label_point_set, auto_label
import itertools
import time
import numpy as np
...
class Cropper:
    def __init__(self):
        ...
        markers_hand.model = pos_dic["pos3_template"]
                ...
                for i in range(n_sub):
                    ...
                    if i == 2:
                        for j in range(pos_dic[list(pos_dic.keys())[i]].shape[1]):
                            ...
                            # cx_temp = int(markers_hand.model[j, 0])
                            # cy_temp = int(markers_hand.model[j, 0])
                            ...
                            if not isinstance(markers_hand.model, np.ndarray):
                                markers_hand.model = np.array(markers_hand.model)
                            pt_tmp.append([markers_hand.model[0][j], markers_hand.model[1][j]])
                            # print(pt_tmp)
                            # plt.figure()
                    ...
                    if i == 2:
                        ...
                        tic = time.time()
                        # ordered_labels = label_point_set(pt_tmp, pt, labels=markers_hand.marker_names)
                        # print(ordered_labels)
                        new_set = minimize_points_location(pt_tmp, pt)
                        ordered_labels = auto_label(new_set, pt, true_labels=markers_hand.marker_names)
                        # markers_hand.model = np.array(pt).T
                        # markers_hand.marker_names = ordered_labels

                        # ordered_labels = auto_label(pt_tmp, pt, true_labels=markers_hand.marker_names)
                        # markers_hand.model = new_set
                        # markers_hand.marker_names = ordered_labels
                        # print(ordered_labels)
                        ...
                        for n in range(len(pt)):
                            ...
                            cv2.putText(self.cropped[i], markers_hand.marker_names[n], (int(new_set[n][0] - 2), int(new_set[n][1] - 2)),
