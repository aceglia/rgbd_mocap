...
def init_participant(biomech_pipeline, part, forces, f_ext, emg, vicon_to_depth, peaks, rt, trial_short, model_directory):
...
def main(model_dir, participants, processed_data_path, source, save_data=True, stop_frame=None,
    ...
    for part, file in zip(mapped_part, all_files):
        ...
        for key in markers_dic.keys():
            ...
            biomech_pipeline.set_stop_frame(stop_frame, dlc_frame_idx, key, live_filter_method[key_counter].value != 0, data_shape=markers_dic[key][1].shape[2])
