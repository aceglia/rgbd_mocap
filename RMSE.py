        ...
        for p, part in enumerate(all_data.keys()):
            ...
            for f, file in enumerate(all_data[part].keys()):
                for j in range(2):
                    ...
                    rmse_file[j, :, f] = compute_error(all_data[part][file]["depth"][key] * factors[k], all_data[part][file][source[idx]][key] * factors[k])
                    std_file[j, :, f] = compute_std(all_data[part][file]["depth"][key] * factors[k], all_data[part][file][source[idx]][key] * factors[k])
            for j in range(2):
                rmse[j, n_key * p: n_key * (p + 1)] = np.mean(rmse_file[j, :, :], axis=1)
                std[j, n_key * p: n_key * (p + 1)] = np.mean(std_file[j, :, :], axis=1)

        print("RMSE for", key, ":", "vicon:",  np.mean(rmse[0, :]), "+/-", np.mean(std[0, :]),
              "minimal_vicon:", np.mean(rmse[1, :]), "+/-", np.mean(std[1, :]))
