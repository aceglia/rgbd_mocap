...
def plot_results(
    ...
    models = [
        f"/mnt/shared/Projet_hand_bike_markerless/process_data/{part}/models/{trial_name}_processed_3_model_scaled_depth_seth.bioMod",
    ...
    for p, plt_name in enumerate(plot_names):
        ...
        if cycle:
            ...
            t = np.linspace(
                0,
                results_from_sources[0][plt_name]["mean"].shape[1],
                results_from_sources[0][plt_name]["mean"].shape[1],
            )
        color = ["b", "r", "g"]
        line = ["-", "-", "-"]
        ...
        for i in range(results_from_sources[0][plt_name]["mean"].shape[0] + 2):
            ...
            for k in range(len(results_from_sources)):
                ...
                ax.plot(
                    t, results_from_sources[k][plt_name]["mean"][count, :] * factor, line[k], color=color[k], alpha=0.7
                )
...
if __name__ == "__main__":
    ...
    for part in all_data.keys():
        for f, file in enumerate(all_data[part].keys()):
            ...
            plot_results(
                all_data[part][file],
                None,  # all_data[part][file]["depth"]["track_idx"],
