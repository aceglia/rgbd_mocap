...
if __name__ == "__main__":
    ...
    for part in all_data.keys():
        ...
        for key in all_data[part].keys():
            ...
            data_tmp["dlc_in_pixel"][:, 8, :], data_tmp["dlc_in_pixel"][:, 9, :] = (
                data_tmp["dlc_in_pixel"][:, 9, :],
                data_tmp["dlc_in_pixel"][:, 8, :],
            )
            fix_len = min(data_tmp["ref_in_pixel"].shape[-1], data_tmp["dlc_in_pixel"].shape[-1])
            error_tmp.append(
                compute_error(data_tmp["ref_in_pixel"][:2, :, :fix_len], data_tmp["dlc_in_pixel"][:2, :, :fix_len])
            )
            std_tmp.append(
                compute_std(data_tmp["ref_in_pixel"][:2, :, :fix_len], data_tmp["dlc_in_pixel"][:2, :, :fix_len])
            )
