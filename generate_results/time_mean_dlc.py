...
if __name__ == "__main__":
    ...
    for key in source_key:
        ...
        all_time_tracking.append(
            [np.round(np.mean(time_tracking) * 1000, 2), np.round(np.mean(std_tracking) * 1000, 2)]
        )
        all_time_tot.append(
            [
                np.round((np.mean(time_ik) + np.mean(time_tracking)) * 1000, 2),
    ...
    \hline
         ...
         \hline
         """
        "\multirow{3}*{Bony landmarks extraction} & 0.8&"
        + f" {all_time_tracking[0][0]: .2f}& {all_time_tracking[0][1]: .2f} "
        + r"\\"
        + "\n"
        " & 0.9&" + f" {all_time_tracking[1][0]: .2f}& {all_time_tracking[1][1]: .2f} " + r"\\" + "\n"
        "& 1.0   &"
        + f" {all_time_tracking[2][0]: .2f}& {all_time_tracking[2][1]: .2f}  "
        + r"\\"
        + "\n"
        + r" \hdashline"
        + "\n"
        "Pose estimation"
        + "& all&"
        + f"{all_time_ik[0][0]: .2f}& {all_time_ik[0][1]: .2f}"
        + r"\\"
        + "\n"
        + r" \hline"
        + "\n"
        ...
        "& 0.8&"
        + r"\textbf{"
        + f"{all_time_tot[0][0]: .2f}"
        + r"} & \textbf{"
        + f"{all_time_tot[0][1]: .2f}"
        + "}"
        + r"\\"
        + "\n"
        "& 0.9  &"
        + r"\textbf{"
        + f"{all_time_tot[1][0]: .2f}"
        + r"} & \textbf{"
        + f"{all_time_tot[1][1]: .2f}"
        + "}"
        + r"\\"
        + "\n"
        "& 1.0    &"
        + r"\textbf{"
        + f"{all_time_tot[2][0]: .2f}"
        + r"} & \textbf{"
        + f"{all_time_tot[2][1]: .2f}"
        + "}"
        + r"\\"
        + "\n"
        + r" \hline"
