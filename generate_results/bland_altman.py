...
def _compute_bland_altman(participants, all_data, files, key, factor, unit, source, to_compare_source):
    ...
    for j in range(len(to_compare_source)):
        ...
        for part, data in zip(participants, all_data):
            ...
            file_counter += 1
            to_compare = data[to_compare_source[j]][key][...,:end_frame] if end_frame is not None else data[to_compare_source[j]][key]
        ...
        print(f"& {comparison_tab[j]} &" + f" {rmse:.2f}& {std:.2f} & {low_limit:.2f}& {high_limit:.2f}& {bias:.2f}" + r"\\")
...
if __name__ == '__main__':
    ...
    all_data, participants = load_all_data(mapped_part, all_files, name_to_load=f"_{file_name[:-4]}_tmp.bio", reload=False)
