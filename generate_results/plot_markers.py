...
data = load("ik.bio")
ik_function = MskFunctions(model="data_files\P4_session2\model_depth_scaled.bioMod",
                           data_buffer_size=data["markers"].shape[2])
ordered_markers = data["markers"]
q, q_dot, qddot = ik_function.compute_inverse_kinematics(ordered_markers,
...
for i in range(3):
    proc_ma[i, :, :] = OfflineProcessing().process_generic_signal(markers[i, :, :],
                                                               moving_average=True,
...
for i in range(markers.shape[2]):
    if i > 0:
        new_value_mat = np.concatenate((new_value_mat,
                                        markers[:, :, i][:, :, np.newaxis]), axis=2)
    else:
