...
class SharedFrames(Frames):
    def __init__(self, color_frame, depth_frame, index=None, downsample_ratio=None, three_chan_color=False):
        super().__init__(color_frame, depth_frame, index, downsample_ratio)
        self.color_shape = (3, self.width, self.height) if three_chan_color else (self.width, self.height)
        self.color_array = RawArray("c", np.prod(self.color_shape))  # 'c' -> value between 0-255
        self.depth_array = RawArray("i", self.width * self.height)  # 'i' -> int32
        self.index = RawValue("i", 0)

        self.color = np.frombuffer(self.color_array, dtype=np.uint8).reshape(self.color_shape)
        self.depth = np.frombuffer(self.depth_array, dtype=np.int32).reshape((self.width, self.height))

        if self.downsample_ratio != 1:
            self.color = cv2.resize(self.color, (self.color_shape))
