...
class SharedFrames(Frames):
    def __init__(self, color_frame, depth_frame):
        ...
        self.color_array = RawArray('c', self.width * self.height * 3)  # 'c' -> value between 0-255
        self.depth_array = RawArray('i', self.width * self.height)  # 'i' -> int32

        self.color = np.frombuffer(self.color_array, dtype=np.uint8).reshape((self.width, self.height, 3))
