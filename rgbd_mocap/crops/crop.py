...
class DepthCheck:
    ...
    def check(pos, depth_image, depth_min, depth_max):
        if DepthCheck.DEPTH_SCALE is None:
            raise ValueError(
                "Please provide the depth scale to have the markers depth in meters."
        ...
        if depth_image[pos[1], pos[0]] > 0:
            ...
            visibility = False
            depth = get_pixels(depth_image, x=pos[1], y=pos[0], delta=DepthCheck.DELTA)
...
class Crop:
    def __init__(
        ...
        if from_dlc:
            ...
            first_markers_position = self.dlc_live.get_pose(self.frame.depth, min_depth=200, bg_remover_threshold=1.2)[
                :, :2
            ]
    ...
    def attribute_depth_from_position(self, positions: list[Position]):
        ...
        for i in range(len(positions)):
            ...
            if isinstance(positions[i], Position):
                depth, visibility = DepthCheck.check(
                    positions[i].position, self.frame.depth, self.depth_min, self.depth_max
                )
                ...
                if (
                    self.marker_set[i].get_depth() == -1
                    or depth != -1
                    and abs(self.marker_set[i].get_depth() - depth) < 0.08
                ):
                    self.marker_set[i].set_depth(depth)
                self.marker_set[i].set_depth_visibility(visibility)
            else:
    ...
    def set_marker_pos(self, positions: List[Position], dlc_enhance_markers=()):
        ...
        for i, mark in enumerate(self.marker_set.markers):
            if (
                mark.name not in self.marker_set.markers_from_dlc
                and mark.name not in self.marker_set.dlc_enhance_markers
            ):
                continue
            # for i in range(len(positions)):
            if (
                self.marker_set[i].is_bounded
                and self.marker_set[i].x_bounds.min == 0
                and self.marker_set[i].y_bounds.min == 0
            ):
