...
class ImageOptions(QWidget):
    ...
    def __init__(self, video_filter, parent=None):
        ...
        self.blend_option = LabeledSlider(
            "Show applied filters (%): ", minimum=0, maximum=100, default_value=self.params["blend"], parent=self
        )
        layout.addWidget(self.blend_option)
        self.blend_option.slider.slider.valueChanged.connect(
            lambda value: self.set_param_value("blend", value) or self.video_filter.update()
        )
    ...
    def link_options(self, options):
        options.activate.pressed.connect(
            lambda: self.set_show_params_value(options.name, options.activate.isChecked()) or self.video_filter.update()
        )
    ...
    def link_option(self, option):
        if isinstance(option, OptionButton):
            option.pressed.connect(
                lambda: (self.set_param_value(option.param, not option.isChecked()) or self.video_filter.update())
            )
        elif isinstance(option, LabeledSlider) or isinstance(option, LabeledRangeSlider):
            option.slider.slider.valueChanged.connect(
                lambda value: (self.set_param_value(option.param, value) or self.video_filter.update())
            )
