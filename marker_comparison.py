...
markers_names = ['test:T5', 'test:C7', 'test:Ribs', 'test:clav_ac','test:scapaa', 'test:scapia'
       , 'test:acrom', 'test:delt', 'test:larm', 'test:epicl',
...
calib_suffix = "11-07-2023_09_16_59"
markers_calib_names = ["wand1", "wand2", "wand3", "wand4"]
markers_vicon_calib = Markers.from_c3d(filename=f"calib_wide.c3d", usecols=markers_calib_names)
markers_depth_calib = load(f"markers_{calib_suffix}.bio")
...
optimal_rotation, optimal_translation = homogeneous_transform_optimization(point_test_calib[:, :, 0], point_test_calib_vicon[:, :, 0])
new_markers_depth_calib = np.dot(np.array(optimal_rotation), np.array(point_test_calib[:, :, 0])) + np.array(optimal_translation)
...
for i in range(len(marker_depth_names)):
    ...
    err_markers[i, 0] = np.median(np.sqrt(np.mean(((new_markers_depth_tmp[:, :]*1000 - new_markers_vicon_int_tmp[:3, :]*1000) ** 2), axis=0)))
print(err_markers)
...
for i in range(len(markers_names)):
    ...
    if i < 4:
        ax.scatter(markers_vicon_calib.values[0, i, :] * 0.001, markers_vicon_calib.values[1, i, :] * 0.001, markers_vicon_calib.values[2, i, :] * 0.001, c='g')
...
for i in range(3):
    for j in range(len(marker_depth_names)):
        ...
        if len(nan_index) > 0:
            ...
            new_markers_depth_filter[i, j, :] = new_markers_depth_tmp # OfflineProcessing().butter_lowpass_filter(new_markers_depth_tmp,
            #                                                                           5, 100, 4)
