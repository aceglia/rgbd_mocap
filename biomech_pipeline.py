...
def main(model_dir, participants, processed_data_path, save_data=False, plot=True, results_from_file=False, stop_frame=None,
    ...
    for part in participants:
        ...
        for file in all_files:
            ...
            file_name_to_save = prefix + f"/Projet_hand_bike_markerless/process_data/{part}/result_biomech_{file.split('_')[0]}_{file.split('_')[1]}_{model}_no_root{suffix}.bio"
            all_results = {}
            dlc_times = None
            label_in_pixel = None
            dlc_in_pixel = None
            ...
            if plot:
                ...
                for i in range(all_results[source[0]]["q_raw"].shape[0]):
                    ...
                    plt.plot( t, all_results["dlc_1"]["q_raw"][i, :stop_frame] * 57.3, c=c[1])
                ...
                for i in range(all_results[source[0]]["q_raw"].shape[0]):
                    plt.subplot(4, 4, i + 1)
                    plt.plot(all_results[source[0]]["q_dot"][i, :stop_frame] * 57.3, c=c[0])
                    plt.plot( t, all_results["dlc_1"]["q_dot"][i, :stop_frame] * 57.3, c=c[1])
                    plt.plot(all_results["minimal_vicon"]["q_dot"][i, :stop_frame] * 57.3, c=c[2])
                    plt.plot(all_results["vicon"]["q_dot"][i, :stop_frame] * 57.3, c=c[3])
                plt.legend([source[0], "dlc_1", "minimal_vicon"])
                plt.show()
            if save_data:
