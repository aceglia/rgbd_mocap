...
class Cropper:
    def __init__(self):
        ...
        self.cap = cv2.VideoCapture("Cycle-Camera 1 (C11141).avi")
        ...
        while i < 3:
            ...
            if not self.cropping:
                ...
                if len(self.x_end) != 0 and len(self.y_end) != 0:
                    cv2.rectangle(f, (self.x_start[i], self.y_start[i]), (self.x_end[i], self.y_end[i]), (255, 0, 0), 2)
                    cv2.imshow("select area, use q when happy", f)
                    i += 1
            elif self.cropping:
                cv2.rectangle(f, (self.x_start[i], self.y_start[i]), (self.x_end[i], self.y_end[i]), (255, 0, 0), 2)
                cv2.imshow("select area, use q when happy", f)
            ...
            if cv2.waitKey(25) & 0xFF == ord('q'):
                ...
                break
            if len(self.x_end) != 0:
                self.cropped.append(self.oriImage[self.y_start[i]:self.y_end[i], self.x_start[i]:self.x_end[i]])
        ...
        while(True):
            ...
            if self.ret:
                #crop frame
                for i in range(3):
                    self.cropped[i] = self.frame[self.y_start[i]:self.y_end[i], self.x_start[i]:self.x_end[i]]
                # Write the frame into the file 'output.avi'
                # self.out.write(self.cropped)

                # Display the resulting frame - trying to move window, but does not always work
                cv2.namedWindow('producing video0', cv2.WINDOW_NORMAL)
                cv2.namedWindow('producing video1', cv2.WINDOW_NORMAL)
                cv2.namedWindow('producing video2', cv2.WINDOW_NORMAL)
                ...
                cv2.imshow('producing video0', self.cropped[0])
                cv2.imshow('producing video1', self.cropped[1])
                cv2.imshow('producing video2', self.cropped[2])
    ...
    def mouse_crop(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            self.x_start.append(x)
            self.y_start.append(y)
            self.x_end.append(x)
            self.y_end.append(y)
            self.cropping = True
