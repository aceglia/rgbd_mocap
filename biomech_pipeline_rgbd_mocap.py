    ...
    for part in participants:
        ...
        for file in all_files:
            ...
            if plot:
                ...
                id_depth = compute_id(
                    all_results[source[0]]["q_raw"][:, :stop_frame], biorbd.Model(model_path), f_ext.copy()
                )
                model_path = f"{model_dir}/{part}/models/{file.split('_')[0]}_{file.split('_')[1]}_model_scaled_minimal_vicon_new_seth.bioMod"
                id_minimal = compute_id(
                    all_results["minimal_vicon"]["q_raw"][:, :stop_frame], biorbd.Model(model_path), f_ext.copy()
                )
                model_path = f"{model_dir}/{part}/models/{file.split('_')[0]}_{file.split('_')[1]}_model_scaled_vicon_new_seth.bioMod"
                id_vicon = compute_id(
                    all_results["vicon"]["q_raw"][:, :stop_frame], biorbd.Model(model_path), f_ext.copy()
                )
                plt.figure("q")
