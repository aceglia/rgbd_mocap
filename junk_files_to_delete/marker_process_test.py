...
if __name__ == '__main__':
    ...
    new_cluster_dlc = ScapulaCluster(measurements[0], measurements[1], measurements[2], measurements[3],
                                 measurements[4], measurements[5], calibration_matrix)
    new_cluster_vicon = ScapulaCluster(measurements[0], measurements[1], measurements[2], measurements[3],
                                 measurements[4], measurements[5], calibration_matrix)
    ...
    for participant in participants:
        for trial in trials:
            ...
            names = names[:names.index("clavac") + 1] + ["scapaa", "scapia", "scapts"] + names[names.index("clavac") + 1:]
            trials = [f"/mnt/shared/Projet_hand_bike_markerless/RGBD/{part}/{trial}/reoriented_dlc_markers.bio"]
            trials = ["/mnt/shared/Projet_hand_bike_markerless/process_data/P14/result_biomech_gear_20_normal_times_three_filtered_all_dofs.bio"]
            data = load(trials[0])
            n_final = 1000
            ...
            dlc_data_tmp = np.concatenate((dlc_data[:, :idx_cluster + 1, :], anato_from_cluster[:3, ...],
                                           dlc_data[:, idx_cluster + 1:, :]), axis=1)
            ...
            off_line_processing = [RealTimeProcessing(60,150),
            ...
            dlc_kalman_origin = np.zeros((new_markers_dlc.shape[0], new_markers_dlc.shape[1], new_markers_dlc.shape[2] + 1))
            ...
            for i in range(frame_idx[0], frame_idx[-1]):
                ...
                if i in frame_idx:
                    ...
                    dlc_data_tmp = None
                def get_next_frame_from_kalman(kalman_instance=None, markers_data=None, scapula_cluster=None, params=None,
                                               measurement_noise_factor=100, process_noise_factor=5,
                                               error_cov_post_factor=0, error_cov_pre_factor=0, rt_matrix=None,
                                               n_markers=None, idx_cluster=None):
                    dt_forward = params[-1]
                    ...
                    if markers_data is not None:
                        ...
                        markers_data = np.concatenate((markers_data[:, :idx_cluster+1, :], anato_from_cluster[:3, ...], markers_data[:, idx_cluster+1:, :]), axis=1)
                    for k in range(n_markers):
                        if kalman_instance[k] is None and markers_data is not None:
                            ...
                            kalman_instance[k] = Kalman(markers_data[:, k, 0], n_measures=3, n_states=6, fps=60, measurement_noise_factor = measurement_noise_factor,
                ...
                #     0.3, 1, 0, None)
                params = [1.638819672361114, 1.9736804302698414, 1.9995932412272837, 1.997539532071001, 1.9633889310240709, 1.947695851912196, 1.9983647882075208, 1.998880629129683, 1.0807187795620514, 4.207208092362829e-09, 3.5408032064211595e-08, 2.123146633494158e-07, 2.0224394293459974e-08, 2.0433951366906184e-07, 0.3279000418749911, 0.20710480248051769, 0.8314056671836689, 0.9645683465216266, 0.330460439129379, 0.31702080456872317, 0.3089735874774765, 0.31183420065408335, 0.5073711499437568, 0.07494298104933172, 0.08953478141229149, 1.8994140162349553, 1.9487702387110173, 1.9915625624652324, 1.9450477411601077, 1.9947866648465944, 1.9988631829827175, 1.9248515992949242, 1.9997968625673925, 1.384130678267083, 0.5557883657741526, 0.5989681804784023, 2.24320551786683e-06, 0.009263235812780353, 1.0413250682013571e-07, 5.233418799794384e-05, 7.510003165591523e-07, 7.16208633911263e-08, 6.0177453360661156e-05, 0.6549354227360592, 0.8089013075729085, 0.5166585170062755, 0.2317465900941644, 0.4731827570928632, 0.038686491506018564, 0.00023974017702647945, 1.081763942496813, 1.202737507528403, 0.05013168718178318, 0.1372350579551961, 0.1903270100841814, 0.3308146651182463, 0.48928150602486353, 0.8026742677662071, 0.7029133518222641, 0.24114616678663592, 0.3673173711175879, 0.6638955719628095, 0.02663505418121219, 0.8095724772390148, 0.8763711023774112, 0.3050450802169265, 0.04490876818500625, 0.034099402450974634]
                ...
                params = [np.float64(472.2654813238043), np.float64(756.2779949201798), np.float64(735.7860415491991), np.float64(928.257097263313), np.float64(812.3799212070905), np.float64(624.6674124294418), np.float64(320.7942179798739), np.float64(301.97788341855954), np.float64(150.1271863662071), np.float64(0.0), np.float64(2.3502986537575554e-07), np.float64(0.0), np.float64(0.0), np.float64(0.0), np.float64(8.085662969304732), np.float64(365.01600884838007), np.float64(187.13848345862235), np.float64(634.5588267260474), np.float64(581.5482308984457), np.float64(667.260408273373), np.float64(518.012710069234), np.float64(376.08358042113787), np.float64(597.5116838219593), np.float64(49.92763599728841), np.float64(51.75065413899098), np.float64(876.8450036999939), np.float64(346.36507845434517), np.float64(974.6280672835907), np.float64(760.9912663133331), np.float64(329.7660030544987), np.float64(626.8214488193189), np.float64(725.0076998770165), np.float64(591.166289491217), np.float64(357.96908186961497)]
            ...
            for i in range(3):
                dlc_reproc[i, :8, :] = OfflineProcessing().butter_lowpass_filter(
                    dlc_kalman[i, :8, :],
                    1, 60, 2)
                dlc_reproc[i, 8:, :] = OfflineProcessing().butter_lowpass_filter(
                    dlc_kalman[i, 8:, :],
